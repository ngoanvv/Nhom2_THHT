...
class Products extends MY_Controller
{
    ...
    public function pro_bycategory($id,$alias){
        ...
        $config['total_rows'] = $this->f_productmodel->count_ProbyCate($alias);
        ...
        $data['lists'] = $this->f_productmodel->getProbyCate($alias,$config['per_page'], $this->input->get('per_page'));
        $data['banners'] = $this->f_productmodel->get_data('images',array(
            'type' => 'page'
        ),array('sort' => 'asc'));
        $data['types'] = $this->f_productmodel->get_data('product_hangsx',array('lang' => $this->language));
    ...
    public function detail($var1,$var2)
    {
		...
		$data['cate_all']=$this->f_productmodel->get_data('product_category',array('lang' => $this->language ),null);
        ...
        $data['pros'] = $this->f_productmodel->getProductSimilar($item->id,$item->category_id,16,0);
        $data['banners'] = $this->f_productmodel->get_data('images',array(
            'type' => 'page'
        ),array('sort' => 'asc'));
