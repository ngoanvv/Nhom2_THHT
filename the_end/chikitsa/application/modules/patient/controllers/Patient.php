...
class Patient extends CI_Controller {
				...
				if(isset($file_upload['error']) && $file_upload['error']!='<p>You did not select a file to upload.</p>'){
					...
					if(isset($patient_id) && $patient_id!=NULL){
						...
						if (in_array("alert", $active_modules)) {				
							...
							redirect("alert/send/new_patient/$patient_id/0/0/0/patient/index/0/0/0");
						}else{
	...
	function bill_detail_report() {
		...
		if (!$this->session->userdata('user_name') || $this->session->userdata('user_name') == '') {
            ...
            if ($this->form_validation->run() === FALSE) {
				...
				$data['reports'] = $this->patient_model->get_bill_report($data['bill_from_date'],$data['bill_to_date']);
	...
	function email_bill($visit_id,$patient_id = NULL){
		...
		if (in_array("alert", $active_modules)) {				
			//Send Alert : bill
			redirect('alert/send/new_bill/0/0/0/'.$visit_id.'/patient/bill/'.$visit_id.'/'.$patient_id.'/0');
		}else{
			$this->bill($visit_id,$patient_id);
		}
	}
