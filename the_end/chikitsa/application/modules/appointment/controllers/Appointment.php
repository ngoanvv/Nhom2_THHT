		...
		if (!$this->session->userdata('user_name') || $this->session->userdata('user_name') == '') {
			...
			if ($this->form_validation->run() === FALSE){
				...
				if (in_array("alert", $active_modules)) {
					...
					redirect('alert/send/new_appointment/0/0/'.$appointment_id.'/0/appointment/index/'.$year.'/'.$month.'/'.$day);
				}else{
		...
		if (!$this->session->userdata('user_name') || $this->session->userdata('user_name') == '') {	
			...
			if($new_status == "Cancel"){
				...
				if (in_array("alert", $active_modules)) {
					...
					redirect('alert/send/appointment_cancel/0/0/'.$appointment_id.'/0/appointment/index/'.$year.'/'.$month.'/'.$day);
				}else{
					redirect('appointment/index/'.$year.'/'.$month.'/'.$day);
				}
			}else{
